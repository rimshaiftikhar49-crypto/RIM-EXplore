          ...
          {stats.map((stat, index) => (
            <div key={index} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-200 card-hover">
              <div className="flex items-center justify-between mb-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${index === 0 || index === 1 ? 'bg-yellow-100' : 'bg-gray-100'
                  }`}>
                  <stat.icon className={`w-5 h-5 ${index === 0 || index === 1 ? 'text-yellow-600' : 'text-gray-600'
                    }`} />
                </div>
